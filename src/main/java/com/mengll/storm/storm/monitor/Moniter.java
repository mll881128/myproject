...
public class Moniter {
	...
	public static void main(String[] args) {
		...
		try {
			zkKeeper = new ZooKeeper(model.getZkConnectString(),model.getZkTimeout(),new Watcher(){
				@Override
...
//			supervisor_num=4;
			if (supervisor_num != model.getSupervisorNum()) {
				//supervisor个数不足，告警出来
				System.out.println(model.getSuper_num_alarm().replaceAll("\\$\\{num}", supervisor_num+""));
				alarmString = model.getSuper_num_alarm().replaceAll("\\$\\{num}", supervisor_num+"") ;
				SMS_oracle.SentAlarm(model, alarmString);
				SMS_oracle.closeCon() ;
				zkKeeper.close(); 
				System.exit(2) ;
			...
			for(String topo:model.getTopologys())
			{
				...
				if (! topoNameList.contains(topo)) {
					//topo 已经停止，告警出来
					alarmString = model.getTopo_name_alarm().replaceAll("\\$\\{name}", topo) ;
