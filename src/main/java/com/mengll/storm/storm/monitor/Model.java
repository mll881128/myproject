package com.mengll.storm.storm.monitor;
...
//				System.out.println(nlist.item(i).getFirstChild().getNodeValue());
			...
			e = (Element) nlist.item(0);
			this.setNimbus_alarm(e.getElementsByTagName("alarm").item(0).getFirstChild().getNodeValue());
//			System.out.println(this.getNimbus_alarm());
			
			nlist = doc.getElementsByTagName("Supervisor");
			e = (Element) nlist.item(0);
			String Supervisor_sum = e.getElementsByTagName("Supervisor-sum").item(0).getFirstChild().getNodeValue() ;
			if (Supervisor_sum != null) {
				setSupervisorNum(Integer.parseInt(Supervisor_sum));
			}
//			System.out.println(this.getSupervisorNum());
			this.setSuper_num_alarm(e.getElementsByTagName("alarm").item(0).getFirstChild().getNodeValue()) ;
...
//			MatchLogic = 
			...
			for (int i = 0; i < topo_num; i++) {
				topo_name = e.getElementsByTagName("topology-name").item(i).getFirstChild().getNodeValue();
				if (topo_name != null) {
					topologys.add(topo_name);
				}
			}
			
			this.setTopo_name_alarm(e.getElementsByTagName("alarm").item(0).getFirstChild().getNodeValue()) ;
//			System.out.println(this.getTopo_name_alarm());
			
			nlist = doc.getElementsByTagName("zookeeper");
			e = (Element) nlist.item(0);
			String host = e.getElementsByTagName("host").item(0).getFirstChild().getNodeValue() ;
			if(host != null)
			{
				setZkConnectString(host) ;
			}
//			System.out.println(getZkConnectString());
			String timeout =  e.getElementsByTagName("timeout").item(0).getFirstChild().getNodeValue() ;
			if (timeout != null) {
...
//			System.out.println(this.getZkTimeout());
	...
	public static void main(String[] args) {
		Model monitor = new Model("storm-monitor-config.xml") ;
